			  ...
			  <li class="nav-item">
			    <a class="nav-link" id="social-tab" data-toggle="tab" href="#social" role="tab" aria-controls="social"
			      aria-selected="false">Facebook & Google Pixels</a>
	   				...
	   				@if(\Request::is('*edit'))
						<form method="post" action="/admin/service/{{$content->id}}" enctype="multipart/form-data">
	   					@method('patch')
	   				@else
					  ...
					  <div class="tab-pane fade show active mt-3 pt-3" id="home" role="tabpanel" aria-labelledby="home-tab">
						<form>
						  <div class="form-group">
						    <label for="options-about text-danger">*Title</label>
						     <input type="text" class="form-control" id="title" name="title" aria-describedby="title" placeholder="Service Title" value="{{$content->title ?? ''}}">
						  </div>
						</form>
					  	...
					  	<textarea class="form-control" id="desc" name="desc" rows="50" cols="50">
					  		{!!$content->desc ?? '' !!}
					  	</textarea>		
						<button type="submit" class="btn btn-success btn-md text-center">Add | Update</button>
					   </div>
					  <div class="tab-pane fade mt-3 pt-3" id="upload" role="tabpanel" aria-labelledby="upload-tab">
					   	<div class="row d-flex justify-content-center">
					   		<div class="col-md-7">
					   			@if(isset($content->picture_1))
					   			<img src="{{asset('storage/'.$content->picture_1)}}" style="max-height: 350px" class="img-responsive img-fluid rounded" >
					   			@endif
					   		</div>
					   		<div class="col-md">
